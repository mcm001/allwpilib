...
class SwerveDriveOdometry {
 public:
   ...
   */
  template <typename... ModuleStates>
  const Pose2d& Update(const SwerveModuleState& moduleState,
                       ModuleStates&&... moduleStates) {
    const auto now = std::chrono::system_clock::now().time_since_epoch();
    units::second_t time{now};
    return UpdateWithTime(time, moduleState, moduleStates...);
  }
  ...
  const Pose2d& Update(units::radians_per_second_t angularVelocity,
    ...
    const auto now = std::chrono::system_clock::now().time_since_epoch();
    units::second_t time{now};
    return UpdateWithTime(time, angularVelocity, moduleStates...);
  }
