...
LTVDiffDriveController::LTVDiffDriveController(const LinearSystem<2, 2, 2>& plant,
                        const std::array<double, 5>& controllerQ,
                        const std::array<double, 2>& controllerR,
                        const DifferentialDriveKinematics& kinematics,
                        units::second_t dt)
    : m_plant(plant), m_rb(kinematics.trackWidth / 2.0), m_kinematics(kinematics) {
  Reset();
  ...
  Eigen::Matrix<double, 5, 5> A0 =
      frc::NumericalJacobianX<10, 10, 2>([this] (auto& x, auto& u) { return Dynamics(x, u); }, x0, u0).block<5, 5>(0, 0);
  Eigen::Matrix<double, 5, 5> A1 =
      frc::NumericalJacobianX<10, 10, 2>([this] (auto& x, auto& u) { return Dynamics(x, u); }, x1, u0).block<5, 5>(0, 0);
  
  m_B =
      frc::NumericalJacobianU<10, 10, 2>([this] (auto& x, auto& u) { return Dynamics(x, u); }, x0, u0).block<5, 2>(0, 0);

  m_K0 = frc::LinearQuadraticRegulator<5, 2>(A0, m_B, controllerQ, controllerR, dt).K();
  m_K1 = frc::LinearQuadraticRegulator<5, 2>(A1, m_B, controllerQ, controllerR, dt).K();
}
...
void LTVDiffDriveController::SetTolerance(const Pose2d& poseTolerance,
...
const Vector<2>& LTVDiffDriveController::Calculate(
    ...
    stateRef << desiredState.pose.Translation().X().to<double>(),
                ...
                wheelVelocities.left.to<double>(),
                wheelVelocities.right.to<double>();
...
Vector<2> LTVDiffDriveController::Controller(
    ...
    error(State::kHeading, 0) = NormalizeAngle(units::radian_t(error(State::kHeading, 0))).to<double>();
    return K * inRobotFrame * error;
}

Vector<10> LTVDiffDriveController::Dynamics(
    const Vector<10>& x,
    const Vector<2>& u) {
...
units::radian_t LTVDiffDriveController::NormalizeAngle(units::radian_t angle) {
    ...
    const int n_pi_pos = (angle.to<double>() + wpi::math::pi) / 2.0 / wpi::math::pi;
    angle -= units::radian_t(n_pi_pos * 2.0 * wpi::math::pi);

    // Cut off the bottom half of the above range to constrain within
    // (-pi, pi]
    const int n_pi_neg = (angle.to<double>() - wpi::math::pi) / 2.0 / wpi::math::pi;
