...
public class LTVDiffDriveController {
  ...
  public LTVDiffDriveController(LinearSystem<N2, N2, N2> plant,
    ...
    this.m_rb = kinematics.trackWidthMeters / 2;
    ...
    var a0 = NumericalJacobian.numericalJacobianX(Nat.N10(), Nat.N10(), this::getDynamics, x0, u0)
            .getStorage().extractMatrix(0, 5, 0, 5);
    var a1 = NumericalJacobian.numericalJacobianX(Nat.N10(), Nat.N10(), this::getDynamics, x1, u0)
            .getStorage().extractMatrix(0, 5, 0, 5);

    m_B = new Matrix<>(NumericalJacobian.numericalJacobianU(Nat.N10(), Nat.N2(),
            this::getDynamics, x0, u0).getStorage().extractMatrix(0, 5, 0, 2));

    m_K0 = new LinearQuadraticRegulator<N5, N2, N3>(new Matrix<>(a0), m_B,
            controllerQ, controllerR, dtSeconds).getK();
    m_K1 = new LinearQuadraticRegulator<N5, N2, N3>(new Matrix<>(a1), m_B,
            controllerQ, controllerR, dtSeconds).getK();
  }
  ...
  protected Matrix<N6, N1> getGlobalMeasurementModel(Matrix<N10, N1> x, Matrix<N2, N1> u) {
    return new MatBuilder<>(Nat.N6(), Nat.N1()).fill(
        ...
        (x.get(State.kRightVelocity.value, 0) - x.get(State.kLeftVelocity.value, 0)) / (2.0 * m_rb)
    );
  }
  ...
  */
  public boolean atReference() {
    Matrix<N5, N1> error = new Matrix<>(m_r.getStorage().extractMatrix(0, 5, 0, 1)
           .minus(m_observer.getXhat().getStorage().extractMatrix(0, 5, 0, 1)));
   ...
   * AngularVelocityError].
   *
   * @return Matrix [N10, N1] containing the states.
   */
   ...
   *
   * @return Matrix [N3, N1] The outputs.
   */
  public Matrix<N3, N1> getLocalOutputs() {
    return m_localY;
  }
   ...
   *
   * @return Matrix [N6, N1] The outputs.
   */
  public Matrix<N6, N1> getGlobalOutputs() {
    return m_globalY;
  }
   ...
   * </p>
   *
   * @return Matrix[N3, N1] The estimated local outputs.
   */
   ...
   * </p>
   *
   * @return Matrix[N6, N1] The estimated global outputs.
   */
  ...
  @SuppressWarnings("JavadocMethod")
  protected Matrix<N2, N1> getController(Matrix<N10, N1> x, Matrix<N5, N1> r) {
   ...
   */
  public DifferentialDriveMotorVoltages calculate(Pose2d poseRef,
          double linearVelocityRefMetersPerSec,
          double angularVelocityRefRadPerSec) {
    var wheelVelocities = m_kinematics.toWheelSpeeds(
            new ChassisSpeeds(linearVelocityRefMetersPerSec,
                    0,
                    angularVelocityRefRadPerSec));

    if (m_useLocalMeasurements) {
      m_observer.correct(m_appliedU, m_localY);
    } else {
      m_observer.correct(Nat.N6(), m_appliedU, m_globalY,
              this::getGlobalMeasurementModel, m_globalR);
    }
    ...
    // Compute feedforward
    var rdot = new Matrix<N5, N1>((m_nextR.getStorage().extractMatrix(0, 5, 0, 1)
            .minus(m_r.getStorage().extractMatrix(0, 5, 0, 1)))
            .divide(m_dtSeconds));
        ...
        .solve(
           rdot.getStorage().minus(
                getDynamics(m_r, new Matrix<>(new SimpleMatrix(2, 1)))
                            .getStorage().extractMatrix(0, 5, 0, 1))
        ));
