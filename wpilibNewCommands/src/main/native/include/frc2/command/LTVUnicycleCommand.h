 ...
 * @see Trajectory
 */
class LTVUnicycleCommand : public CommandHelper<CommandBase, LTVUnicycleCommand> {
 public:
  /**
   ...
   */
  LTVUnicycleCommand(frc::Trajectory trajectory, std::function<frc::Pose2d()> pose,
   ...
   * feedforwards; outputs are the raw wheel speeds from the LTV Unicycle controller,
   ...
   * Constructs a new LTVUnicycleCommand that, when executed, will follow the
   * provided trajectory. Performs no PID control and calculates no
   * feedforwards; outputs are the raw wheel speeds from the LTVUnicycle controller,
   * and will need to be converted into a usable form by the user.
   *
   * @param trajectory      The trajectory to follow.
   * @param pose            A function that supplies the robot pose - use one of
